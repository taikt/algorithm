...
public:
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        ...
        FOR(i,0,n) {
            if (!visited[i]) {
                set<string> temp; // use set to avoid insert duplicated emails
    ...
    void visitDFS(int i, vector<vector<string>>& accounts, set<string>& temp, vector<bool>& visited, unordered_map<string,set<int>>& emailId) {
        visited[i] = true;
